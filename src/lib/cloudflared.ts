...
function ensureDirs() {
  if (!existsSync(BETTERTUNNELS_DIR))
    mkdirSync(BETTERTUNNELS_DIR, { recursive: true })
  ...
  try {
    ...
    if (!existsSync(credsPath)) {
      return {
        ...
        authenticated: false,
        message:
          "cloudflared installed but not authenticated. Run `cloudflared tunnel login`.",
      }
    }
...
export function createCloudflareTunnel(name: string) {
  ...
  if (!match)
    throw new Error(`Failed to parse tunnel ID from output: ${output}`)
  return {
    tunnelId: match[1],
    credentialsPath: path.join(homedir(), ".cloudflared", `${match[1]}.json`),
  }
}
...
export async function routeDnsViaApi(
  tunnelId: string,
  domain: string,
  apiToken: string,
) {
  ...
  if (!recordData.success) {
    ...
    if (alreadyExists) {
      ...
      if (!recordId)
        throw new Error(
          `CNAME record exists but could not be found for: ${domain}`,
        )
      ...
      if (!patchData.success) {
        throw new Error(
          `Failed to update existing CNAME record: ${JSON.stringify(patchData.errors)}`,
        )
      }
    } else {
      throw new Error(
        `DNS record creation failed: ${JSON.stringify(recordData.errors)}`,
      )
    }
  }
}
...
export function getCloudflareApiToken(): string {
  ...
  if (!existsSync(certPath))
    throw new Error(
      "cloudflared cert.pem not found. Run `cloudflared tunnel login`.",
    )
  ...
  throw new Error(
    "Could not extract API token from cert.pem. Re-run `cloudflared tunnel login`.",
  )
}
...
export function getCertAccountInfo(): { accountID: string; zoneID: string } {
  const certPath = path.join(homedir(), ".cloudflared", "cert.pem")
  if (!existsSync(certPath))
    throw new Error("cert.pem not found. Run `cloudflared tunnel login`.")
  const raw = readFileSync(certPath, "utf-8")
  const b64 = raw
    .replace(/-----BEGIN[\s\S]*?-----/g, "")
    .replace(/-----END[\s\S]*?-----/g, "")
...
export function startTunnelProcess(
  ...
  if (!existsSync(credentialsPath)) {
    throw new Error(
      `Tunnel credentials not found at ${credentialsPath}. Re-create the tunnel.`,
    )
  }
  ...
  const proc = spawn(
    "cloudflared",
    ["tunnel", "--config", ymlPath, "run", "--protocol", "http2"],
    {
